...
<div id="micontent" class="shown">
  ...
  <div class="properties form large-9 medium-8 columns content">
    ...
    <div id="panel_header">
      <h3><?= __('Editar APP: ') . h($appdata->appname) ?></h3>
      <div id="action-buttons">
        <button id="action-save" class="link-action addlink" type="submit">
    ...
    <div class="container-fluid container-full-blanco container-edit">
      ...
      <div class="col-lg-6 col-md-6 col-sm-12 col-blanca-left">
        ...
        <div class="row mt-2">
          <div class="col-lg-12 col-md-12">
            <p class="dato-titulo"><?= __('Nombre del paquete *') ?></p>
            <input
              type="text"
              name="packagename"
              maxlength="255"
              id="packagename"
              value="<?= h($appdata->packagename) ?>"
              required
            >
          </div>
        </div>
        ...
        <div class="row mt-2">
          <div class="col-lg-6 col-md-12">
            ...
            <select class="selectform" name="devicetype" id="devicetype" required>
              <option value="0" <?= $appdata->devicetype === 0 ? 'selected' : '' ?>><?= __('Android') ?></option>
              <option value="1" <?= $appdata->devicetype === 1 ? 'selected' : '' ?>><?= __('iOS') ?></option>
            </select>
          </div>
          <div class="col-lg-6 col-md-12">
            ...
            <select class="selectform" name="appcategory_es" id="appcategory-es" required>
              <?php foreach (array_keys($catMap) as $key): ?>
                <option value="<?= h($key) ?>" <?= $appdata->appcategory === $key ? 'selected' : '' ?>>
                  <?= h($key) ?>
                </option>
              <?php endforeach; ?>
            </select>
